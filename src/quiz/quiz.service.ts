...
export class QuizService {
  ...
  async update(id: number, updateQuizDto: UpdateQuizDto): Promise<Quiz> {
    ...
    if (updateQuizDto.questions) {
      const updatedQuestions = await Promise.all(updateQuizDto.questions.map(async q => {
        let question = quizFound.questions.find(existingQuestion => existingQuestion.id === q.id);
        if (!question) {
        ...
        if (q.answers) {
          question.answers = q.answers.map(a => {
            let answer = question.answers ? question.answers.find(existingAnswer => existingAnswer.id === a.id) : null;
            if (!answer) {
              answer = new Answer();
            }
            answer.text = a.text;
